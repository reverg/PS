...
Greedy로 O(N), O(K)에 풀린다고 하는데 증명이 어렵다. 아이디어는 문자 중 최대로 나타난 것의
출현 횟수가 M이라고 할 때, M-1개의 segment를 크기 k 이상으로 만들면 된다는 것이다. 최대 횟수로
나타나는 것들 M개 segment에 1개씩 배치 -> 최대 횟수 - 1로 나타나는 것들 앞 M-1개에 넣기 -> 남은
공간에 나머지 문자들 하나씩 균등하게 넣기 작업이 끝났을 때 M-1개 segment 크기가 모두 k 이상이면
성공, segment를 다 합쳐서 return. 안되면 중간에 빈 공간이 생겼다는 뜻이므로 실패.
